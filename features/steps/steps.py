# product/features/steps/steps.py
...
def step_when_add_product_to_product_list(context):
    # Implement the logic to add the product to the product list
    pass

@then('the product "{name}" should be listed in the product list')
...
def step_when_search_for_products(context, name):
    context.search_results = Product.objects.filter(name=name)

@then('I should see the product named "{name}" in the search result')
def step_then_should_see_product_in_search_result(context, name):
...
def step_then_should_see_product_in_list(context, name):
